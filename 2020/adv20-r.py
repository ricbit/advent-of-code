...
def build_rotation_links(g, tiles):
  ...
  for a, b in g.edges:
    a, b = min(a, b), max(a, b)
    tilepairs = itertools.product(enumerate(tiles[a]), enumerate(tiles[b]))
    for (xc, c), (xd, d) in tilepairs:
      if all(x not in [a, b] for x in [xc, xd]):
        sidepairs = itertools.product(get_side(c), get_side(d))
        for (cdir, ckey), (ddir, dkey) in sidepairs:
...
def find_right_rotation(start, rotations):
  for rot in range(8):
    (_, _, adir), (_, _, bdir) = rotations[start, rot]
