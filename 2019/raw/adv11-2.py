...
def draw(field):
  xmin = min(int(f.real) for f, color in field.items() if color == 1)
  xmax = max(int(f.real) for f, color in field.items() if color == 1)
  ymin = min(int(f.imag) for f, color in field.items() if color == 1)
  ymax = max(int(f.imag) for f, color in field.items() if color == 1)
  for j in reversed(range(ymin, 1 + ymax)):
    line = []
