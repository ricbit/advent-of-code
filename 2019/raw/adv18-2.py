...
def newencode(state):
  ...
  #@return (tuple(pos), tuple(sorted(keys)))
  return pos + keys
...
def solve2(t, robots):
  ...
  while vnext:
    ...
    for name, (robot, j, i, doors) in get_available(keys, col_keys):
      ...
      encoded_keys = "".join(sorted(col_keys.union([name])))
      new_hh = hh[:]
      new_hh = [heuristic(newpos[robot], encoded_keys, keys, robot) for robot in range(len(robots))]
